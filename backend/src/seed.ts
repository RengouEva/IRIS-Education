...
const run = async () => {
  ...
  const existingAdmin = await db.get("SELECT id FROM users WHERE email = 'admin@iris.edu'")
  if (existingAdmin) {
  ...
  await db.run('INSERT INTO universities (id, name, country, logo, cover_config) VALUES (?, ?, ?, ?, ?)', uniId1, 'Université Félix Houphouët-Boigny', "Côte d'Ivoire", '/images/university-logo-1.png', coverConfig1)
  await db.run('INSERT INTO universities (id, name, country, logo, cover_config) VALUES (?, ?, ?, ?, ?)', uniId2, 'Université Cheikh Anta Diop', 'Sénégal', '/images/university-logo-2.png', coverConfig2)
  await db.run('INSERT INTO universities (id, name, country, logo, cover_config) VALUES (?, ?, ?, ?, ?)', uniId3, 'Université de Yaoundé I', 'Cameroun', '/images/university-logo-3.png', coverConfig3)

  await db.run('INSERT INTO users (id, firstname, lastname, email, password, role, avatar, universityId) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    adminId, 'Admin', 'IRIS', 'admin@iris.edu', bcrypt.hashSync('admin123', 10), 'admin', '', uniId1)
  await db.run('INSERT INTO users (id, firstname, lastname, email, password, role, avatar, universityId) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    supervisorId, 'Amadou', 'Diallo', 'amadou.diallo@univ.fr', bcrypt.hashSync('password123', 10), 'supervisor', '/images/avatar-user-2.jpg', uniId1)
  await db.run('INSERT INTO users (id, firstname, lastname, email, password, role, avatar, universityId) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    studentId, 'Marie', 'Kouassi', 'marie.kouassi@univ.fr', bcrypt.hashSync('password123', 10), 'student', '/images/avatar-user-3.jpg', uniId1)

  const templateId = crypto.randomUUID()
  await db.run('INSERT INTO templates (id, name, level, description, image) VALUES (?, ?, ?, ?, ?)',
    templateId, 'Mémoire de Master', 'master', 'Template standard pour mémoire de Master avec toutes les sections académiques', '/images/template-master.jpg')
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    await db.run('INSERT INTO template_default_sections (id, templateId, title, type, orderIndex) VALUES (?, ?, ?, ?, ?)',
      sectionId, templateId, s.title, s.type, i)
    if (s.subs) {
      for (let j = 0; j < s.subs.length; j++) {
        await db.run('INSERT INTO template_default_subsections (id, defaultSectionId, title, orderIndex) VALUES (?, ?, ?, ?)',
          crypto.randomUUID(), sectionId, s.subs[j], j)
      }
    }
  }
  ...
  await db.run('INSERT INTO templates (id, name, level, description, image) VALUES (?, ?, ?, ?, ?)',
    templateLicenceId, 'Mémoire de Licence', 'licence', 'Template pour mémoire de Licence avec structure simplifiée', '/images/template-licence.jpg')
  ...
  for (let i = 0; i < licenceSections.length; i++) {
    await db.run('INSERT INTO template_default_sections (id, templateId, title, type, orderIndex) VALUES (?, ?, ?, ?, ?)',
      crypto.randomUUID(), templateLicenceId, licenceSections[i], i === 0 ? 'introduction' : i === licenceSections.length - 2 ? 'conclusion' : i === licenceSections.length - 1 ? 'bibliography' : 'chapter', i)
  }

  const templateDoctoratId = crypto.randomUUID()
  await db.run('INSERT INTO templates (id, name, level, description, image) VALUES (?, ?, ?, ?, ?)',
    templateDoctoratId, 'Thèse de Doctorat', 'doctorat', 'Template complet pour thèse de Doctorat', '/images/template-doctorat.jpg')
  ...
  for (let i = 0; i < doctoratSections.length; i++) {
    await db.run('INSERT INTO template_default_sections (id, templateId, title, type, orderIndex) VALUES (?, ?, ?, ?, ?)',
      crypto.randomUUID(), templateDoctoratId, doctoratSections[i],
  ...
  await db.run('INSERT INTO templates (id, name, level, description, image) VALUES (?, ?, ?, ?, ?)',
    templateStageId, 'Rapport de Stage', 'stage', 'Template pour rapport de stage professionnel', '/images/template-stage.jpg')

  await db.run("INSERT INTO app_config (key, value) VALUES (?, ?) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value", 'appName', 'IRIS-Education')
  await db.run("INSERT INTO app_config (key, value) VALUES (?, ?) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value", 'appDescription', 'Rédaction de mémoires académiques assistée par IA')
  await db.run("INSERT INTO app_config (key, value) VALUES (?, ?) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value", 'contactEmail', 'contact@iris-education.ai')
  await db.run("INSERT INTO app_config (key, value) VALUES (?, ?) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value", 'maxProjectsFree', '3')
  await db.run("INSERT INTO app_config (key, value) VALUES (?, ?) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value", 'aiSuggestionsPerDay', '20')
